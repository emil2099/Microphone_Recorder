...
if __name__ == '__main__':
    ...
    while True:
        ...
        if pj.status.GetButtonEvents()['data']['SW3'] == 'SINGLE_PRESS':
            pj.status.AcceptButtonEvent('SW3')
            print('Pause/resume press detected')
            m.running_status = not m.running_status
        elif pj.status.GetButtonEvents()['data']['SW2'] == 'SINGLE_PRESS':
            pj.status.AcceptButtonEvent('SW2')
            print('SW2 click')
            pj.status.SetLedState('D2', [128, 128, 0])
